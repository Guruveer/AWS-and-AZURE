output "standard" {
  ...
  value = {
    ...
    ###################
    frontend_port_name             = lower("fpn-${var.env}-${var.location-map[var.location]}-${local.subId}")
    frontend_ip_configuration_name = lower("ficn-${var.env}-${var.location-map[var.location]}-${local.subId}")
    http_setting_name              = lower("hsn-${var.env}-${var.location-map[var.location]}-${local.subId}")
    http_listener_name             = lower("hlsn-${var.env}-${var.location-map[var.location]}-${local.subId}")
    request_routing_rule_name      = lower("rrrn-${var.env}-${var.location-map[var.location]}-${local.subId}")
    ssl_cert_name                  = lower("ssl-${var.env}-${var.location-map[var.location]}-${local.subId}")
    listener_name                  = lower("ln-${var.env}-${var.location-map[var.location]}-${local.subId}")
    ###################
    backup-policy                         = lower("bup-${var.env}-${var.location-map[var.location]}-${local.subId}") #added by SAS ILAA engineering team for CCSAS-500
    ...
    capacity-reservation-group            = lower("crg-${var.env}-${var.location-map[var.location]}-${local.subId}")
    ...
    internal-load-balancer-rule           = lower("ilbr-${var.env}-${var.location-map[var.location]}-${local.subId}") #mwg
    ...
    psql-server-flex                      = lower("psql${var.env}${var.location-map[var.location]}${local.subId}") # added part of SAS ILAA on Cloud CCSAS-627
    ...
    virtual-machine-short                 = lower("vm-${var.env}")                                                  #mwg
